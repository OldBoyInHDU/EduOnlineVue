<template>
    <div class="home-container">
        <div class="searchParam" style="padding: 20px">
            <Button type="success" icon="md-add-circle" @click="uploadVidBtn">上传视频</Button>
            ...
            <Input v-model="title" placeholder="请输入视频标题" style="width: 300px;padding-left: 20px" clearable/>
                    ...
                    <Form :model="formItem" :label-width="120">
                        ...
                        <FormItem label="教学视频" required prop="vid" :rules="{required: true, message: '视频不能为空', trigger: 'blur'}">
                            <Upload
                                ...
                                multiple
                                accept="video/mp4"
                                action="http://localhost:8080/eduonline/positionLearning/videoUpload/uploadVideo">
                                <Button icon="ios-cloud-upload-outline">上传视频</Button>
                                <div slot="tip">支持文件类型:.mp4，文件大小不超过200mb</div>
                            </Upload>
                        </FormItem>
                    </Form>
                </Modal>
...
export default {
    ...
    methods: {
        ...
        uploadVidBtn() {
            let that = this
            that.resetForm()
            that.uploadVidModal = true
        },
        handleSuccess (res, file) {
            // console.log(res)
            // console.log('file:' + file.name)
            let that = this
            let vidStoragePath = '/video/' + res.data
            that.formItem.uploadList.push({
                fileName: file.name,
                storagePath: vidStoragePath,
            })
        ...
        submit() {
            ...
            request.post(
                '/positionLearning/videoUpload/submitVideoInfo',
                formData,
                {
                    headers: {
                        'content-type': 'application/json'
        ...
        confirm(index) {
            ...
            that.toDeleteVid = that.form_list_content[index].videoFileName
            that.delVidIndex = index
            // that.remove(index)
        },
        remove() {
            let index = this.delVidIndex
            // console.log(index)
